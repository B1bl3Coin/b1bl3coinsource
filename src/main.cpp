...
unsigned int nTargetSpacing = 1 * 64; // 60 seconds
unsigned int nStakeMinAge = 60 * 60 * 1; // 6 hours
...
const string strMessageMagic = "B1bl3Coin Signed Message:\n";
...
int64_t CTransaction::GetMinFee(unsigned int nBlockSize, enum GetMinFee_mode mode, unsigned int nBytes) const
{
    ...
    int64_t nMinFee = (0 + (int64_t)nBytes / 1000) * nBaseFee;
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    if(pindexBest->nHeight == 6 ) // premine
    {
        nSubsidy = 500000000 * COIN;
    }
    else if(pindexBest->nHeight < 5011 && pindexBest->nHeight > 9)
    {
        nSubsidy =  20000 * COIN;
    }
    else if(pindexBest->nHeight < 10012 && pindexBest->nHeight > 5010)
    {
        nSubsidy =  15000 * COIN;
    }
    else if(pindexBest->nHeight < 20013 && pindexBest->nHeight > 10011)
    {
        nSubsidy =  10000 * COIN;
    }
    else if(pindexBest->nHeight < 30014 && pindexBest->nHeight > 20012)
    {
        nSubsidy =  22500 * COIN;
    }
    else if(pindexBest->nHeight < 45015 && pindexBest->nHeight > 30013)
    {
        nSubsidy =  10000 * COIN;
    }
    else if(pindexBest->nHeight < 60016 && pindexBest->nHeight > 45014)
    {
        nSubsidy =  35000 * COIN;
    }
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "B1bl3Coin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (fTestNet)
    {
        ...
        nStakeMinAge = 8 * 60; // test net min age is 8 min
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "B1bl3Coin coin !2014 ";
        CTransaction txNew;
        txNew.nTime = 1414846938;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1414846938;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 1610736;
		if(fTestNet)
        {
            block.nNonce   = 68152;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x00cfa337523326f92f1ab478843406b6b5adacc2a61e4b79c66bed2e5179f4c3"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xac, 0xca, 0xf7, 0x7c };
